...
func runSpringBootInit(args []string) error {
	...
	fs.Usage = func() {
		fmt.Fprintln(fs.Output(), pluginHelpLine("Usage: cub-gen springboot init [flags] <source-path>"))
		fmt.Fprintln(fs.Output())
		fmt.Fprintln(fs.Output(), pluginHelpLine("Generate starter cub-gen material for a Spring Boot application."))
		fmt.Fprintln(fs.Output())
...
func runSpringBootValidateMutation(args []string) error {
	...
	fs.SetOutput(os.Stderr)
	fs.Usage = func() {
		fmt.Fprintln(fs.Output(), pluginHelpLine("Usage: cub-gen springboot validate-mutation [flags] <field-path>"))
		fmt.Fprintln(fs.Output())
		...
		fmt.Fprintln(fs.Output(), pluginHelpLine("  cub-gen springboot validate-mutation --routes ./operational/field-routes.yaml feature.inventory.reservationMode"))
		fmt.Fprintln(fs.Output())
		fmt.Fprintln(fs.Output(), "  # Blocked (platform-owned)")
		fmt.Fprintln(fs.Output(), pluginHelpLine("  cub-gen springboot validate-mutation --routes ./operational/field-routes.yaml spring.datasource.url"))
		fmt.Fprintln(fs.Output())
		fmt.Fprintln(fs.Output(), "Flags:")
		fs.PrintDefaults()
	}
...
func runSpringBootSetEmbeddedConfig(args []string) error {
	...
	fs.SetOutput(os.Stderr)
	fs.Usage = func() {
		fmt.Fprintln(fs.Output(), pluginHelpLine("Usage: cub-gen springboot set-embedded-config [flags] <field-path> <value>"))
		fmt.Fprintln(fs.Output())
		...
		fmt.Fprintln(fs.Output(), pluginHelpLine("  cub-gen springboot set-embedded-config --routes ./operational/field-routes.yaml --file ./confighub/inventory-api-prod.yaml --configmap inventory-api-config feature.inventory.reservationMode optimistic"))
		fmt.Fprintln(fs.Output())
		fmt.Fprintln(fs.Output(), "  # Blocked: platform-owned datasource field")
		fmt.Fprintln(fs.Output(), pluginHelpLine("  cub-gen springboot set-embedded-config --routes ./operational/field-routes.yaml --file ./confighub/inventory-api-prod.yaml --configmap inventory-api-config spring.datasource.url jdbc:postgresql://shadow.platform.svc:5432/inventory"))
		fmt.Fprintln(fs.Output())
		fmt.Fprintln(fs.Output(), "Flags:")
		fs.PrintDefaults()
	}
...
func printSpringBootUsage(out io.Writer) {
	fmt.Fprintln(out, pluginHelpLine("Usage: cub-gen springboot <subcommand> [flags]"))
	fmt.Fprintln(out)
	fmt.Fprintln(out, "Spring Boot onboarding and enforcement commands.")
	fmt.Fprintln(out)
	fmt.Fprintln(out, "Subcommands:")
	fmt.Fprintln(out, pluginHelpLine("  init               Generate starter cub-gen material for a Spring Boot app"))
	...
	fmt.Fprintln(out)
	fmt.Fprintln(out, pluginHelpLine("Run 'cub-gen springboot <subcommand> --help' for details."))
}
