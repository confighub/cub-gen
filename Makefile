...
build-plugin:
	mkdir -p .tmp/cub-plugin/plugins/gen
	go build -o .tmp/cub-plugin/plugins/gen/main ./cmd/cub-gen

plugin-smoke: build-plugin
	CUB_CONFIG="$(CURDIR)/.tmp/cub-plugin/config.yaml" CONFIGHUB_AGENT=1 cub gen --help
