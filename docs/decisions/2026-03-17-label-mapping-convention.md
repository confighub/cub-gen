...
- The examples repo's `hub.yaml` uses `environment` and `region` labels on targets, but nothing connects these to the labels found on cluster resources.
...
And AppSpaces are organized by team (`platform-team`, `payments-team`). But there's no declared mapping from "ArgoCD Application with label `team=payments`" → "import into space `payments-team`".
...
- cub-scout `import_argocd.go` — current ArgoCD import (no label mapping)
