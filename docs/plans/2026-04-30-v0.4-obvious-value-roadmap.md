...
- [x] [#236](https://github.com/confighub/cub-gen/issues/236) Ship cub-gen as `cub gen` plugin
...
Current state: the repo can stage `cub-gen` as `$CUB_CONFIG/plugins/gen/main`,
`cub gen --help` dispatches through the real `cub` plugin host in local proof,
plugin mode renders `cub gen ...` help, and release packaging is configured to
ship the `main` plugin entry point. Public `cub plugin install confighub/cub-gen`
still depends on cutting the next plugin-compatible release artifact.
