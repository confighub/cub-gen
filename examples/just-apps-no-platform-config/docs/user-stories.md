...
The checkout team needs a cancellations channel. They edit `ably.yaml`, adding a new channel entry. `cub-gen` detects the change, traces the new field back to the DRY source, and produces inverse-edit guidance: "to change cancellation channel config in production, edit `ably-prod.yaml` channels section."

## 2. Production channel rename

Ops renames the EU inbound channel in `ably-prod.yaml`. The import shows this as an overlay edit with clear lineage: base channel name from `ably.yaml`, overridden by prod-specific value. The change bundle captures both the before and after state.
