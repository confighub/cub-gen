...
usage() {
  cat <<'USAGE'
Usage:
  ./examples/demo/app-ai-change-run.sh <repo-path> [render-target]

Environment variables:
  SPACE         ConfigHub space label (default: platform)
  VERIFIER      Attestation verifier label (default: ai-assistant)
  OUT_DIR       Output directory (default: .tmp/app-ai-change-run/<repo>-<timestamp>)
...
echo "[change-run] mutation card"
cat "$out_dir/mutation-card.json"

echo "[change-run] artifacts: $out_dir"
